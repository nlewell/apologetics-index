...
const CSV_COLUMNS: (keyof SpreadsheetExportRow)[] = [
  'generalTopic',
  'subtopic',
  'charge',
  'searchQuery',
...
@Injectable()
export class ContentSpreadsheetService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly configService: ConfigService,
  ) {}

  async exportCsv(): Promise<ExportResponse> {
    ...
    const indexedItemsByQuery = new Map<string, YoutubeSearchResult[]>();

    for (const row of indexedSearchRows) {
      const parsedItems = Array.isArray(row.items)
        ? row.items
            .map((item) => this.coerceYoutubeSearchResult(item as Prisma.JsonValue))
    ...
    const pinnedRowsByQuery = new Map<string, typeof pinnedRows>();
    ...
    const exportRows: SpreadsheetExportRow[] = [];
    ...
    for (const indexRow of indexRows) {
      ...
      const mergedByVideoId = new Map<
        string,
        { item: YoutubeSearchResult; source: 'search_index' | 'pinned_override' }
      >();
  ...
  async importCsv(csv: string): Promise<ImportResponse> {
    ...
    const groupedRows = new Map<string, ImportRowWithMeta[]>();
    ...
    for (const [, rowsForKey] of groupedRows.entries()) {
      ...
      const sourceKey = existingIndexItem?.sourceKey ?? `manual-${randomUUID()}`;

      const indexData: Prisma.ApologeticIndexItemUncheckedCreateInput = {
        sourceKey,
        generalTopic: firstRow.generalTopic,
        subtopic: firstRow.subtopic,
        charge: firstRow.charge,
      ...
      const videoRows = rowsForKey.filter(
        (entry) => entry.row.videoId || entry.row.videoUrl,
      );

      const searchQuery =
        firstRow.searchQuery ||
      ...
      for (const entry of videoRows) {
        ...
        let hydratedVideo: HydratedYoutubeVideo;

        try {
          hydratedVideo = await this.fetchVideoMetadataById(resolvedVideoId);
        } catch (error) {
          throw new BadRequestException(
            `Row ${entry.rowNumber}: failed to hydrate metadata for video ${resolvedVideoId}. ${this.getErrorMessage(error)}`,
          );
        }
  ...
  private normalizeImportRow(row: Record<string, string | undefined>): SpreadsheetImportRow {
    const cleanedText = (value: string | undefined): string | null => {
      const trimmed = (value ?? '').trim();
      return trimmed.length > 0 ? trimmed : null;
    };

    const cleanedBoolean = (value: string | undefined): boolean | null => {
      if (value === undefined) {
        return null;
      }
    ...
    const cleanedNumber = (value: string | undefined): number | null => {
      const trimmed = (value ?? '').trim();
      if (!trimmed) {
        return null;
      }
  ...
  private rowToYoutubeSearchResult(
    row: SpreadsheetImportRow,
    hydratedVideo: HydratedYoutubeVideo,
  ): YoutubeSearchResult {
  ...
  private coerceYoutubeSearchResult(value: Prisma.JsonValue): YoutubeSearchResult | null {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      return null;
    }

    const row = value as Partial<YoutubeSearchResult>;
  ...
  private stringifyCsv(rows: SpreadsheetExportRow[]): string {
    const header = CSV_COLUMNS.join(',');
    const lines = rows.map((row) =>
      CSV_COLUMNS.map((column) => this.escapeCsvValue(row[column])).join(','),
    );
  ...
  private buildTopicPathQuery(
    ...
    subtopicValue: string | null,
    chargeValue: string | null,
  ): string {
    const normalizeSegment = (value: string | null) => (value ?? '').replace(/\s+/g, ' ').trim();
  ...
  private async fetchVideoMetadataById(videoId: string): Promise<HydratedYoutubeVideo> {
    const apiKey = this.configService.get<string>('YOUTUBE_API_KEY');

    if (!apiKey) {
      throw new BadRequestException('YOUTUBE_API_KEY is required to hydrate imported video metadata.');
    }
    ...
    if (!response.ok) {
      ...
      throw new BadRequestException(
        `YouTube metadata lookup failed (${response.status}) for ${videoId}: ${text}`,
      );
    }

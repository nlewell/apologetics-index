...
let YoutubeService = class YoutubeService {
    ...
    officialAnswerCache = new Map();
    queryInsightCache = new Map();
    shortsMaxSeconds;
    ...
    async getOfficialChurchAnswer(input) {
        ...
        if (!generateIfMissing) {
            return {
                ...
                answerSource: null,
                answerSnippet: null,
                rationale: 'Official church answer link is not cached yet. Open this top match to generate and cache one.',
                confidenceScore: 0,
            };
        }
        ...
        for (const candidateQuery of topicQueryCandidates) {
            ...
            try {
                ...
                candidateSelection = await this.selectOfficialChurchAnswerWithAi(candidateQuery, candidates);
                candidateMatch =
                    candidateSelection.matchFound && candidateSelection.selectedUrl
                        ? candidates.find((candidate) => this.normalizeComparableUrl(candidate.url) ===
                            this.normalizeComparableUrl(candidateSelection.selectedUrl ?? '')) ?? null
                        : null;
            }
            catch {
                hadLookupError = true;
        ...
        if (!matchedCandidate) {
            ...
            if (curatedCandidate) {
                ...
                selection = {
                    ...
                    rationale: 'Matched from curated official-topic mapping when external search candidates were weak or noisy.',
                    confidenceScore: 0.9,
                };
            }
        }
    ...
    buildOfficialChurchTopicQueryCandidates(topicQuery) {
        ...
        const strippedQuestionLead = normalizedWhitespace.replace(/^(what is|what are|who is|who are|explain|define)\s+/i, '');
    ...
    async getCuratedOfficialChurchCandidate(topicQuery) {
        const normalized = this.normalizeText(topicQuery);
        if (normalized.includes('priesthood')) {
            const url = 'https://www.churchofjesuschrist.org/study/manual/gospel-topics/priesthood?lang=eng';
    ...
    async getQueryInsight(input) {
        ...
        const matchedCandidate = aiPayload.bestUrl
            ? candidates.find((candidate) => this.normalizeComparableUrl(candidate.url) ===
                this.normalizeComparableUrl(aiPayload.bestUrl ?? '')) ?? null
            : null;
        const response = {
            topicQuery,
            cacheStatus: 'generated',
            ...
            bestSourceRationale: aiPayload.bestSourceRationale?.trim() ||
    ...
    buildOfficialChurchAnswerCacheKey(videoId, topicQuery) {
        return `${videoId}|${this.normalizeText(topicQuery)}`;
    }
    async getCachedOfficialChurchAnswer(cacheKey) {
    ...
    setCachedOfficialChurchAnswer(cacheKey, response) {
        this.officialAnswerCache.set(cacheKey, {
            expiresAt: Date.now() + this.commentsAnalysisCacheTtlMs,
            response,
        });
    }
    async getPersistedOfficialChurchAnswer(cacheKey) {
    ...
    async setPersistedOfficialChurchAnswer(cacheKey, videoId, topicQuery, response) {
        const refreshedAt = new Date();
        const toStore = {
            ...response,
            cacheStatus: 'generated',
    ...
    coerceOfficialChurchAnswerResponse(value) {
        if (!value || typeof value !== 'object') {
            return null;
        }
        const row = value;
    ...
    buildQueryInsightCacheKey(topicQuery) {
        return this.normalizeText(topicQuery);
    }
    async getCachedQueryInsight(cacheKey) {
    ...
    setCachedQueryInsight(cacheKey, response) {
        this.queryInsightCache.set(cacheKey, {
            expiresAt: Date.now() + this.commentsAnalysisCacheTtlMs,
            response,
        });
    }
    async getPersistedQueryInsight(cacheKey) {
        let row = null;
        try {
    ...
    async setPersistedQueryInsight(cacheKey, topicQuery, response) {
        const refreshedAt = new Date();
        const toStore = {
            ...response,
            cacheStatus: 'generated',
    ...
    coerceQueryInsightResponse(value) {
        if (!value || typeof value !== 'object') {
            return null;
        }
        const row = value;
    ...
    async fetchWebSearchCandidates(topicQuery) {
        ...
        if (!response.ok) {
            const text = await response.text();
            throw new common_1.BadGatewayException(`Web search request failed (${response.status}): ${text}`);
        }
    ...
    async analyzeQueryInsightWithAi(topicQuery, candidates) {
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You summarize likely answers to a topic query from provided search result snippets and choose the single best URL for further reading. Return strict JSON only. Do not invent facts beyond the provided candidates. If the candidates are weak, still provide a cautious answer and leave bestUrl empty if none is strong enough.',
                    },
                    {
                        role: 'user',
                        content: [
        ...
        if (!response.ok) {
            const text = await response.text();
            throw new common_1.BadGatewayException(`AI query insight request failed (${response.status}): ${text}`);
        }
        const payload = (await response.json());
        const content = payload.choices?.[0]?.message?.content;
        if (!content) {
            throw new common_1.BadGatewayException('AI query insight response was empty.');
        }
        try {
            return JSON.parse(content);
        }
        catch {
            throw new common_1.BadGatewayException('AI query insight returned invalid JSON.');
        }
    }
    resolveWebSearchResultUrl(rawUrl) {
        try {
    ...
    async fetchOfficialChurchSearchCandidates(topicQuery) {
        ...
        if (!response.ok) {
            const text = await response.text();
            throw new common_1.BadGatewayException(`Church search request failed (${response.status}): ${text}`);
        }
        const html = await response.text();
    ...
    async fetchOfficialChurchSearchCandidatesViaWeb(topicQuery) {
        ...
        const resultSets = await Promise.allSettled(scopedQueries.map((query) => this.fetchWebSearchCandidates(query)));
    ...
    async fetchOfficialChurchSearchCandidatesViaBing(topicQuery) {
        ...
        while ((itemMatch = itemRegex.exec(xml)) !== null) {
            ...
            const normalizedUrl = this.resolveOfficialChurchResultUrl(this.decodeHtmlEntities(rawLink.trim()));
            if (!normalizedUrl ||
                seen.has(normalizedUrl) ||
                !this.isAllowedOfficialChurchResult(normalizedUrl)) {
                continue;
            ...
            const title = this.normalizeWhitespace(this.decodeHtmlEntities(this.stripHtml(rawTitle)));
            if (!title || title.length < 4 || this.isIgnoredOfficialChurchTitle(title)) {
                continue;
            }
            const rawDescription = descriptionRegex.exec(itemXml)?.[1] ?? '';
            const snippet = this.normalizeWhitespace(this.decodeHtmlEntities(this.stripHtml(rawDescription)));
            seen.add(normalizedUrl);
            candidates.push({
                title,
                url: normalizedUrl,
    ...
    async fetchOfficialChurchCandidateSnippet(url) {
        try {
            const response = await fetch(url);
            if (!response.ok) {
                return '';
            ...
            const html = await response.text();
            const metaDescriptionMatch = /<meta[^>]+(?:name|property)="(?:description|og:description)"[^>]+content="([^"]+)"/i.exec(html) ??
    ...
    async selectOfficialChurchAnswerWithAi(topicQuery, candidates) {
        if (!candidates.length) {
            return {
                matchFound: false,
                rationale: 'No official church results were available to evaluate.',
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You choose official Church of Jesus Christ of Latter-day Saints links for a topic. Return strict JSON only. Choose one candidate only if it directly answers or clearly explains the topic from an official church source. If the results are only tangentially related, return matchFound false.',
                    },
                    {
                        role: 'user',
                        content: [
        ...
        if (!response.ok) {
            const text = await response.text();
            throw new common_1.BadGatewayException(`AI official answer request failed (${response.status}): ${text}`);
        }
        const payload = (await response.json());
        const content = payload.choices?.[0]?.message?.content;
        if (!content) {
            throw new common_1.BadGatewayException('AI official answer response was empty.');
        }
        try {
            return JSON.parse(content);
        }
        catch {
            throw new common_1.BadGatewayException('AI official answer returned invalid JSON.');
        }
    }
    resolveOfficialChurchResultUrl(rawUrl) {
    ...
    isAllowedOfficialChurchResult(urlValue) {
        try {
    ...
    isIgnoredOfficialChurchTitle(title) {
    ...
    getOfficialChurchSourceLabel(urlValue) {
        try {
    ...
    normalizeComparableUrl(urlValue) {
        try {
            const url = new URL(urlValue);
            url.hash = '';
